 ...
 * items 3-5). Signing happens inline inside submit() rather than as a separate signatures call.
 */
final class EvaluationService
{
    ...
    /**
     * RULE-EVAL-03: the sum of all criteria max scores must equal the template max score.
     *
    ...
    public static function validateCriteriaSum(float $templateMaxScore, array $criteriaMaxScores): void
    {
        ...
        if (abs($sum - $templateMaxScore) > 0.001) {
            throw new AuthException(
                'VALIDATION_ERROR',
                sprintf('ผลรวมคะแนนหัวข้อย่อย (%.2f) ต้องเท่ากับคะแนนเต็มของแบบประเมิน (%.2f)', $sum, $templateMaxScore),
    ...
    /**
     * Admin console for editing the existing evaluation templates and criteria.
     *
     * @return array<int, array{id:int,name:string,evaluator_type:string,max_score:float,criteria:array<int,array{id:int,criteria_name:string,max_score:float}>}>
     */
    ...
    /**
     * RULE-EVAL-03 is enforced here again before any write occurs.
     *
     * @param array<int, array{name:string,max_score:float}> $criteriaUpdates
     */
    public function updateTemplateCriteria(int $templateId, float $newTemplateMax, array $criteriaUpdates): void
    {
        ...
        foreach ($criteriaUpdates as $criteriaId => $c) {
            ...
            if ($name === '') {
                throw new AuthException('VALIDATION_ERROR', 'กรุณากรอกชื่อหัวข้อประเมินให้ครบทุกข้อ');
            }
    ...
    public function getTemplateByType(string $evaluatorType): ?array
    {
        $templates = $this->client->restGet(
            ...
            'evaluator_type=eq.' . $evaluatorType . '&is_active=eq.true&select=id,max_score&order=id.asc'
        );
        foreach ($templates as $template) {
            $criteria = $this->client->restGet(
                'evaluation_criteria',
                'template_id=eq.' . $template['id'] . '&select=id,criteria_name,max_score&order=sort_order.asc'
            );

            // A duplicate, unfinished template must never render as a 0/0 assessment form.
            if ($criteria !== []) {
                return [
                    'id' => (int) $template['id'],
    ...
    private function listEligible(string $internshipFilter, string $evaluatorType, bool $isWeekly): array
    {
        ...
        try {
            $internships = $this->client->restGet('internships', $query);
        ...
        foreach ($internships as $i) {
            ...
            $out[] = [
                'internship_id' => (int) $i['id'],
                'student_name' => $studentName,
                'student_code' => (string) ($student[0]['student_code'] ?? '-'),
                'company_name' => $companyName,
                'internship_status' => (string) ($i['status'] ?? '-'),
                'next_week' => $nextWeek,
     ...
     * authoring time), RULE-EVAL-04 (signature required), and RULE-EVAL-05 (submitted evaluations are locked).
     * enforced by refusing to touch a row whose status is already 'submitted').
     *
     * Flow: create the evaluation, write its scores, create the signature, then mark it submitted.
     ...
     * The evaluation is created before the signature so the signature can reference its real identifier.
     ...
     */
        ...
        $template = $this->getTemplateByType($evaluatorType);
        if ($template === null) {
            throw new AuthException('VALIDATION_ERROR', 'ไม่พบแบบประเมินที่รองรับสำหรับรายการนี้');
        }

        if ($evaluatorType === 'company_weekly' && $weekNumber === null) {
            throw new AuthException('VALIDATION_ERROR', 'กรุณาระบุสัปดาห์ที่ต้องการประเมิน');
        }

        $criteriaById = [];
        foreach ($template['criteria'] ?? [] as $criterion) {
            $criteriaById[(int) $criterion['id']] = [
                'criteria_name' => (string) ($criterion['criteria_name'] ?? 'หัวข้อประเมิน'),
                'max_score' => (float) ($criterion['max_score'] ?? 0),
            ];
        }
        if ($criteriaById === []) {
            throw new AuthException('VALIDATION_ERROR', 'ยังไม่ได้ตั้งค่าหัวข้อการประเมินสำหรับแบบฟอร์มนี้');
        }

        $existing = $this->findEvaluation($internshipId, (int) $template['id'], $weekNumber);
        if ($existing !== null && ($existing['status'] ?? '') === 'submitted') {
            throw new AuthException('VALIDATION_ERROR', 'แบบประเมินนี้ถูกส่งเรียบร้อยแล้วและไม่สามารถแก้ไขซ้ำได้');
        }

        $scoreRows = [];
        $total = 0.0;
        foreach ($criteriaById as $criteriaId => $criterion) {
            if (!array_key_exists((string) $criteriaId, $scores) && !array_key_exists($criteriaId, $scores)) {
                throw new AuthException('VALIDATION_ERROR', 'กรุณากรอกคะแนนให้ครบทุกหัวข้อ');
            }

            $rawScore = $scores[(string) $criteriaId] ?? $scores[$criteriaId];
            if (!is_numeric($rawScore)) {
                throw new AuthException('VALIDATION_ERROR', 'คะแนนที่กรอกไม่ถูกต้อง');
            }

            $score = round((float) $rawScore, 2);
            if ($score < 0 || $score > $criterion['max_score']) {
                throw new AuthException('VALIDATION_ERROR', 'คะแนนของหัวข้อ "' . $criterion['criteria_name'] . '" ต้องอยู่ระหว่าง 0 ถึง ' . $criterion['max_score']);
            }
        ...
        try {
            ...
            $this->logSubmissionFailure('persist_evaluation', $e, [
                'internship_id' => $internshipId,
                'template_id' => (int) $template['id'],
                'week_number' => $weekNumber,
                'evaluator_id' => $evaluatorUserId,
        ...
        if ($evaluationId <= 0) {
            error_log('[evaluation.submit] persist_evaluation returned no evaluation id');
            throw new AuthException('EVALUATION_SAVE_FAILED', 'ไม่สามารถบันทึกข้อมูลแบบประเมินได้ กรุณาลองใหม่อีกครั้ง');
        }

        $fullScoreRows = array_map(
            static fn (array $scoreRow) => ['evaluation_id' => $evaluationId, 'criteria_id' => $scoreRow['criteria_id'], 'score' => $scoreRow['score']],
            $scoreRows
        );
    ...
    private function logSubmissionFailure(string $stage, \Throwable $e, array $context = []): void
    {
        error_log(sprintf(
            '[evaluation.submit] stage=%s message=%s context=%s',
            $stage,
            $e->getMessage(),
    ...
    /** Reserved hook for a future combined final score. */
    ...
    /** Returns submitted evaluations, respecting the student score-visibility setting. */
     ...
     * is the same enforcement level every other admin-only action in this codebase uses so far.
     * Every call is written to audit_logs regardless of outcome intent (old/new value).
     */
    public function adminOverrideScores(int $evaluationId, array $scores, string $adminUserId): void
    {
        ...
        if (!isset($rows[0])) {
            throw new AuthException('VALIDATION_ERROR', 'ไม่พบแบบประเมินที่ต้องการแก้ไข');
        }

        $templateId = (int) $rows[0]['template_id'];
        $criteria = $this->client->restGet('evaluation_criteria', 'template_id=eq.' . $templateId . '&select=id,criteria_name,max_score&order=sort_order.asc');
        $criteriaById = [];
        ...
        $newRows = [];
        $total = 0.0;
        foreach ($scores as $criteriaIdRaw => $scoreRaw) {
            $criteriaId = (int) $criteriaIdRaw;
            if (!isset($criteriaById[$criteriaId])) {
                throw new AuthException('VALIDATION_ERROR', 'พบหัวข้อประเมินที่ไม่มีอยู่ในระบบ');
            }
            ...
            $maxScore = (float) $criteriaById[$criteriaId]['max_score'];
            if ($score < 0 || $score > $maxScore) {
                throw new AuthException('VALIDATION_ERROR', 'คะแนนของหัวข้อ "' . $criteriaById[$criteriaId]['criteria_name'] . '" ต้องอยู่ระหว่าง 0 ถึง ' . $maxScore);
            }
